...
<script src="/crm2/js/jquery-3.2.1.min.js"></script>
...
function initialise() {
	...
	$.post("/crm2/FindRoleRightServlet",function(data){
		var arr=new Array();
		arr=data.split("-");
       l2ID = theMenu.addEntry(-1, "Folder", "客户关系管理系统", "/crm2/html/mywork.jsp", "客户关系管理系统");
       if(arr[0]=="true"||arr[1]=="true"){
		L01 = theMenu.addChild(l2ID, "Folder", "营销管理", "", "营销管理...");
		}
	
	if(arr[0]=="true"){
	L0101 = theMenu.addChild(L01, "Document", "销售机会管理", "ShowAllSaleServlet", "销售机会管理...");
	}
	if(arr[1]=="true"){
	L0102 = theMenu.addChild(L01, "Document", "客户开发计划", "DevServlet", "客户开发计划...");
	}
	...
	if(arr[2]=="true"){
	L0201 = theMenu.addChild(L02, "Document", "客户信息管理", "AllCustServlet", "客户信息管理...");
	}
	if(arr[3]=="true"){
	L0201 = theMenu.addChild(L02, "Document", "客户流失管理", "AllLostServlet1", "客户流失管理...");
	}	
	...
	if(arr[4]=="true"){
	L0301 = theMenu.addChild(L03, "Document", "服务创建", "html/~cust/service/add.jsp", "服务创建...");
	}
	if(arr[5]=="true"){
	L0302 = theMenu.addChild(L03, "Document", "服务分配", "LoadServlet?action=Onload&status=1", "服务分配...");
	}
	if(arr[6]=="true"){
	L0303 = theMenu.addChild(L03, "Document", "服务处理", "LoadServlet?action=dealBy&status=2", "服务处理...");
	}
	if(arr[7]=="true"){
	L0304 = theMenu.addChild(L03, "Document", "服务反馈", "LoadServlet?action=feedBack&status=3", "服务反馈...");
	}
	if(arr[8]=="true"){
	L0305 = theMenu.addChild(L03, "Document", "服务归档", "LoadServlet?action=arch&status=4", "服务归档...");
	}
	...
	if(arr[9]=="true"){
	L0401 = theMenu.addChild(L04, "Document", "客户贡献分析", "OrderListServlet", "客户贡献分析...");
	}
	if(arr[10]=="true"){
	L0402 = theMenu.addChild(L04, "Document", "客户构成分析", "LevelServlet", "客户构成分析...");
	}
	if(arr[11]=="true"){
	L0403 = theMenu.addChild(L04, "Document", "客户服务分析", "ServiceServlet", "客户服务分析...");
	}
	if(arr[12]=="true"){
	L0404 = theMenu.addChild(L04, "Document", "客户流失分析", "LostServlet", "客户流失分析...");
	}
	if(arr[13]=="true"||arr[14]=="true"||arr[15]=="true"){
	L05 = theMenu.addChild(l2ID, "Folder", "基础数据", "", "基础数据...");
	}
	if(arr[13]=="true"){
	L0501 = theMenu.addChild(L05, "Document", "数据字典管理", "DictServlet", "数据字典管理...");
	}
	if(arr[14]=="true"){
	L0502 = theMenu.addChild(L05, "Document", "查询产品信息", "ProductServlet", "查询产品信息...");
	}
	if(arr[15]=="true"){
	L0503 = theMenu.addChild(L05, "Document", "查询库存", "StorageServlet", "查询库存...");
	}
	if(arr[16]=="true"||arr[17]=="true"){
        L06 = theMenu.addChild(l2ID, "Folder", "权限管理", "", "权限管理...");
       }
    if(arr[16]=="true"){
	L0601 = theMenu.addChild(L06, "Document", "用户管理", "AllUserServlet", "用户管理...");
	}
	if(arr[17]=="true"){
	L0602 = theMenu.addChild(L06, "Document", "角色授权管理", "AllRoleServlet", "角色授权管理...");
	}
